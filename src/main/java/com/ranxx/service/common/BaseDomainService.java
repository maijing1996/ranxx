package com.ranxx.service.common;
...
public abstract class BaseDomainService<T extends BaseDomain, K extends MyMapper<T>> {
	...
	public boolean save(T t) throws BusinessException {
		try {
			...
			ex.printStackTrace();
			String type = t.getRoom_id() != null ? "保存" : "新建";
	...
	public boolean deleteByIds(List<T> recordList) throws BusinessException {
		...
		if (recordList != null) {
			for (T t : recordList) {
				idList.add(String.valueOf(t.getRoom_id()));
			}
		}
